...
// --- Client-Side Component: Contains all interactive logic ---
// This component will be rendered on the client, inside a Suspense boundary.
function SearchToolClient() {
  // All hooks and client-side logic are safe to use here.
  'use client';
...
// --- 主页面组件 (服务器组件) ---
// 这个组件现在非常简单，它的唯一职责是提供 Suspense 边界
export default function HomePage() {
  const COMPANY_LOGO_PATH = '/HLG-logo.png';
  
  // Suspense Fallback UI: 在客户端组件加载时显示的占位符
  const Fallback = () => (
    <div className="flex-grow flex justify-center items-center">
      <p className="text-gray-500 text-xl font-semibold">加载中...</p>
    </div>
  );
  ...
  return (
    <div className="min-h-screen flex flex-col items-center bg-white p-4 sm:p-8 pt-16 sm:pt-20">
      ...
      {/* 【修复】使用 Suspense 包裹客户端组件。
        这会告诉 Next.js，被包裹的 SearchToolClient 组件依赖于客户端信息（如 URL 参数），
        应在客户端渲染。这解决了 useSearchParams 的构建错误。
      */}
